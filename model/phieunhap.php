...
function addCTPhieuNhap($idphieunhap, $idsach, $soluong,  $dongia, $loinhuan){
    $conn = connectdb();
    if($conn){
        $sql = 'INSERT INTO chitietphieunhap(idphieunhap, idsach, soluong, dongia, loinhuan) VALUE (?,?,?,?,?)';
        $sttm = $conn->prepare($sql);
        return $sttm->execute([$idphieunhap, $idsach, $soluong, $dongia, $loinhuan]);
    }
    return null;
}
