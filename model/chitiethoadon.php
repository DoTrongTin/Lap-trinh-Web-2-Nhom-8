<?php
        ...
        if($conn){
            try {
                $sql = "SELECT cthd.idhoadon, s.tensach, s.idtacgia, s.idnhaxuatban, s.idctdanhmuc,
                    cthd.idsach, cthd.soluong, cthd.dongia, (cthd.soluong * cthd.dongia) AS thanhtien
    ...
    function addChiTietHoaDon($idhoadon, $idsach, $soluong, $gia) {
        try {
            $conn = connectdb();
            $sql = "INSERT INTO chitiethoadon (idhoadon, idsach, soluong, dongia) VALUES (:idhoadon, :idsach, :soluong, :dongia)";
            $sttm = $conn->prepare($sql);
            $sttm->bindValue(':idhoadon', $idhoadon, PDO::PARAM_INT);
