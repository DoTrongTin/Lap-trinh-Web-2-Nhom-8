...
function getDataSanPhamPhieuNhap(){
    ...
    if($conn){
        //load data
        $sql = "select * from banphim WHERE trangthai =1";
...
function getDataSanPham($page =1, $limit=4){
    ...
    if($conn){
        ...
        $offset = ($page -1)*$limit;
        //đếm số sản phẩm
        $sqlCount = "select count(*) as total from banphim ";
        $sttm = $conn->prepare($sqlCount);
        $sttm->execute();
        $totalRecords = $sttm->fetch(PDO::FETCH_ASSOC)['total'];
        $totalPages = ceil($totalRecords/$limit);
        //load data
        $sql = "select * from banphim  LIMIT :limit OFFSET :offset";
        $sttm = $conn->prepare($sql);
...
function delSanPhamByID($idbanphim){
    $conn = connectdb();
    if($conn){
        try{
            $sql = "UPDATE banphim SET trangthai = 0 WHERE idbanphim = :idbanphim";
            $sttm = $conn->prepare($sql);
...
function getDataSanPhamTheoId($idbanphim){
    $conn = connectdb();
    if($conn){
        try{
            $sql = "SELECT * from banphim WHERE idbanphim = :idbanphim ";
            $sttm =$conn->prepare($sql);
            $sttm->bindValue(':idbanphim', $idbanphim, PDO::PARAM_INT);
            $sttm->execute()>0;
...
function updateSanPhamById($idbanphim, $trangthai){
    $conn = connectdb();
    if($conn){
        try{
            $sql = "UPDATE banphim SET trangthai= :trangthai WHERE idbanphim=:idbanphim";
            $sttm =$conn->prepare($sql);
...
function themSach($tenSach, $idTacGia, $idNXB, $idCTDanhMuc, $moTa, $anhbia) {
    ...
    if ($conn) {
        try {
            ...
            $stmt = $conn->prepare("
                INSERT INTO banphim (tenbanphim, idthuonghieu, idnhaphaphoi, idctdanhmuc, gia, sltonkho, mota, anhbia, trangthai) 
...
function kiemTraIdSachTrongChiTietHoaDon($idbanphim){
    $conn = connectdb();
    if($conn){
        $sql = "SELECT count(*) FROM chitiethoadon WHERE idbanphim =:idbanphim";
        $sttm =$conn->prepare($sql);
        $sttm->bindValue(':idbanphim', $idbanphim, PDO::PARAM_INT);
        $sttm->execute();
...
function delSanPhamTheoIdComplete($idbanphim){
    $conn = connectdb();
    if($conn){
        try{
            $sql = "DELETE FROM banphim WHERE idbanphim = :idbanphim";
            $sttm = $conn->prepare($sql);
            $sttm->bindValue(':idbanphim', $idbanphim, PDO::PARAM_INT);
            $success = $sttm->execute();
            if($success && $sttm->rowCount() > 0){
...
function checkTrungTenSach($tenSach) {
    ...
    if ($conn) {
        try {
            $sql = "SELECT COUNT(*) FROM banphim WHERE tenbanphim = :tenSach";
            ...
            error_log("Lỗi khi kiểm tra trùng tên sản phẩm: " . $e->getMessage());
            return false;
        }
    }
    return false;
...
function searchSanPham($keyword, $page, $limit) {
    ...
    $countSql = "SELECT COUNT(*) FROM banphim WHERE tenbanphim LIKE :keyword OR idbanphim LIKE :keyword";
    ...
    $sql = "SELECT * FROM banphim WHERE tenbanphim LIKE :keyword OR idbanphim LIKE :keyword 
...
function updateSoLuongTheoId($idbanphim, $sltonkho){
    $conn = connectdb();
    if($conn){
        try{
            $sql = "UPDATE banphim SET sltonkho = :sltonkho WHERE idbanphim = :idbanphim";
            $sttm = $conn->prepare($sql);
...
function updateDonGiaTheoId($idbanphim, $dongia){
    $conn = connectdb();
    if($conn){
        try{
            $sql = "UPDATE banphim SET gia = :gia WHERE idbanphim = :idbanphim";
            $sttm = $conn->prepare($sql);
            $sttm->bindValue(':gia', $dongia, PDO::PARAM_INT);
            $sttm->bindValue(':idbanphim', $idbanphim, PDO::PARAM_INT);
            $success = $sttm->execute();
            if($success && $sttm->rowCount() > 0){
                return true;
...
function updateSlTonKhoTheoId($idbanphim, $sltonkho){
    $conn = connectdb();
    if($conn){
        $sql = "UPDATE banphim SET sltonkho = :sltonkho WHERE idbanphim = :idbanphim";
        $sttm = $conn ->prepare($sql);
...
function getSoLuongTonKho($idbanphim) {
    $conn = connectdb();
    if ($conn) {
        try {
            $sql = "SELECT sltonkho FROM banphim WHERE idbanphim = :idbanphim AND trangthai = 1";
            $stmt = $conn->prepare($sql);
            $stmt->bindValue(':idbanphim', $idbanphim, PDO::PARAM_INT);
            $stmt->execute();
...
function updateSoLuongTonKho($idbanphim, $soLuong) {
    $conn = connectdb();
    if ($conn) {
        try {
            $sql = "UPDATE banphim SET sltonkho = sltonkho - :soLuong WHERE idbanphim = :idbanphim AND trangthai = 1";
            $stmt = $conn->prepare($sql);
            $stmt->bindValue(':soLuong', $soLuong, PDO::PARAM_INT);
            $stmt->bindValue(':idbanphim', $idbanphim, PDO::PARAM_INT);
            $success = $stmt->execute();
            if ($success && $stmt->rowCount() > 0) {
                error_log("Cập nhật tồn kho thành công: idbanphim=$idbanphim, soLuong=$soLuong");
                return true;
            } else {
                error_log("Cập nhật tồn kho thất bại: idbanphim=$idbanphim, soLuong=$soLuong, không có hàng được cập nhật");
                return false;
            }
        } catch (PDOException $e) {
