...
function addCTPhieuNhap($idphieunhap, $idbanphim, $soluong,  $dongia, $loinhuan){
    $conn = connectdb();
    if($conn){
        $sql = 'INSERT INTO chitietphieunhap(idphieunhap, idbanphim, soluong, dongia, loinhuan) VALUE (?,?,?,?,?)';
        $sttm = $conn->prepare($sql);
        return $sttm->execute([$idphieunhap, $idbanphim, $soluong, $dongia, $loinhuan]);
    }
    return null;
}
...
function getDataPhieuNhapTheoKhoangThoiGian($page, $limit, $startDate, $endDate) {
    $conn = connectdb();
    if ($conn) {
        // Lấy trang bắt đầu phân trang
        $offset = ($page - 1) * $limit;

        // Đếm số lượng phiếu nhập trong khoảng thời gian
        $sqlCount = "SELECT count(*) AS total FROM phieunhap WHERE ngaynhap BETWEEN :startDate AND :endDate";
        $sttm = $conn->prepare($sqlCount);
        ...
        $sql = "SELECT * FROM phieunhap WHERE ngaynhap BETWEEN :startDate AND :endDate LIMIT :limit OFFSET :offset";
        $sttm = $conn->prepare($sql);
        $sttm->bindValue(':startDate', $startDate, PDO::PARAM_STR);
        $sttm->bindValue(':endDate', $endDate, PDO::PARAM_STR);
        $sttm->bindValue(':limit', $limit, PDO::PARAM_INT);
