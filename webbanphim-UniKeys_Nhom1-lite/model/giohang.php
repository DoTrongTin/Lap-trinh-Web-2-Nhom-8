...
function addSanPhamVaoGioHang($idKhachHang, $idSach, $soLuong) {
    $conn = connectdb();
    try {
        $sqlCheck = "SELECT idgiohang, soluong FROM giohang WHERE idkhachhang = ? AND idbanphim = ? AND trangthai = 1";
        ...
        if ($existingItem) {
            ...
            $sqlInsert = "INSERT INTO giohang (idkhachhang, idbanphim, soluong, trangthai) VALUES (?, ?, ?, 1)";
...
function checkSoLuongSanPhamTrongGioVoiTonKho($idSach, $soLuongYeuCau) {
    $conn = connectdb();
    try {
        $sql = "SELECT sltonkho FROM banphim WHERE idbanphim = ?";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$idSach]);
