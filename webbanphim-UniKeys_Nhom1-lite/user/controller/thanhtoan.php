    ...
    private function placeOrder() {
        try {
            ...
            foreach ($cartItems as $item) {
                if (!isset($item['idbanphim']) || !isset($item['soluong'])) {
                    $conn->rollBack();
                    return json_encode(['status' => 'error', 'message' => 'Dữ liệu giỏ hàng không hợp lệ']);
                }

                // Kiểm tra số lượng tồn kho
                $slTonKho = getSoLuongTonKho($item['idbanphim']);
                if ($slTonKho < $item['soluong']) {
                    $conn->rollBack();
                    return json_encode([
                        'status' => 'error',
                        'message' => "Sản phẩm ID {$item['idbanphim']} không đủ tồn kho (còn $slTonKho, yêu cầu {$item['soluong']})"
                    ]);
                }

                $sanpham = getDataSanPhamTheoId($item['idbanphim']);
                ...
                $result = addChiTietHoaDon($idhoadon, $item['idbanphim'], $item['soluong'], $gia);
                if (!$result) {
                    $conn->rollBack();
                ...
                $updateTonKho = updateSoLuongTonKho($item['idbanphim'], $item['soluong']);
                if (!$updateTonKho) {
                    $conn->rollBack();
                    return json_encode([
                        'status' => 'error',
                        'message' => "Cập nhật tồn kho cho sản phẩm ID {$item['idbanphim']} thất bại"
                    ]);
                }
            }
