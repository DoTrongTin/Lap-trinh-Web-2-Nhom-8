...
switch ($page) {
    ...
    case 'chitietsp':
        ...
        if (isset($_POST['add_to_cart']) && isset($_SESSION['idkhachhang'])) {
            ...
            $stmt = $conn->prepare("SELECT sltonkho FROM banphim WHERE idbanphim = ? AND trangthai = 1");
            $stmt->execute([$bookId]);
            $book = $stmt->fetch(PDO::FETCH_OBJ);
            if ($book && $book->sltonkho >= $quantity) {
                $stmt = $conn->prepare("
                    INSERT INTO giohang (idkhachhang, idbanphim, soluong, trangthai)
                    VALUES (?, ?, ?, 1)
                ...
                echo '<script>alert("Số lượng vượt quá tồn kho hoặc sản phẩm không tồn tại!"); window.location.href = "index.php?pg=chitietsp&id=' . $bookId . '";</script>';
            }
