...
$(document).ready(function() {
            ...
            success: function(response) {
                if (response.status === 'success') {
                    ...
                    response.data.forEach(function(item) {
                        ...
                        let cartItemHtml = `
                            <div class="item-cart-detail" data-idgiohang="${item.idgiohang}" data-idbanphim="${item.idbanphim}">
                                <div class="box-img-cart-detail">
                                    <a href="../controller/index.php?pg=chitietsp&id=${item.idbanphim}">
                                        <img src="../../${item.anhbia}" alt="${item.tenbanphim}">
                                    </a>
                                </div>
                                <div class="box-name-cart-detail">
                                    <h1><a href="../controller/index.php?pg=chitietsp&id=${item.idbanphim}">${item.tenbanphim}</a></h1>
    ...
    $(document).on('click', '.increase-btn, .decrease-btn', function() {
        ...
        let idSach = $item.data('idbanphim'); 
