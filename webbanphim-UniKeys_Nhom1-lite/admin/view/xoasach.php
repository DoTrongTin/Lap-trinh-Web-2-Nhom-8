...
$idbanphim = isset($_GET['idbanphim']) ? $_GET['idbanphim'] : null;

if ($idbanphim) {
    $product = getDataSanPhamTheoId($idbanphim);
    $isInHoaDon = kiemTraIdSachTrongChiTietHoaDon($idbanphim);

    if ($product['trangthai'] == 1) {
        $result = delSanPhamByID($idbanphim); 
        if ($result) {
            echo "<script>
        ...
        if (delSanPhamTheoIdComplete($idbanphim)) {
            echo "<script>
