        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            if ($idPhieuNhap) {
                ...
                foreach ($chiTietPhieuNhap as $chiTiet) {
                    addCTPhieuNhap($idPhieuNhap, $chiTiet['idSach'], $chiTiet['soLuong'], $chiTiet['donGia'],  $chiTiet['loiNhuan']);
                    ...
                    $giacu = $sanpham['gia'];
                    $giamoi =  (($chiTiet['loiNhuan'] /100) * ($chiTiet['donGia'])) + $chiTiet['donGia'];
                    if($giacu < $giamoi){
        ...
        <div class="container">
            ...
            <div class="right-pane">
                <form action="../controller/index.php?pg=themphieunhap" method="post" id="themvaogio">
                    ...
                    <div class="form-group">
                        <label for="IDSach"><b>ID sản phẩm:</b></label>
                        <input type="text" id="IDSach" name="IDSach" readonly>
                    </div>

                    <div class="form-group">
                        <label for="tenSach"><b>Tên sản phẩm:</b></label>
                        <input type="text" id="tenSach" name="tenSach" readonly>
                    </div>

                    <div class="form-group">
                        <label for="tacGia"><b>Thương hiệu:</b></label>
                        <input type="text" id="tacGia" name="tacGia" readonly>
                    </div>

                    <div class="form-group">
                        <label for="nxb"><b>Nhà phân phối:</b></label>
...
<script>
    ...
    function themvaogiohang() {
        ...
        const thanhTien = (parseFloat(donGia) * parseInt(soLuong)).toLocaleString('vi-VN');
        ...
        const row = `
            <tr>
                ...
                <td>${nxb}</td>
                <td>${danhMuc}</td>
                <td>${Number(donGia).toLocaleString('vi-VN')}đ</td>
                <td>${soLuong}</td>
                <td>${loiNhuan}%</td>
                <td>${thanhTien}đ</td>
                <td>
    ...
    for (let row of rows) {
        ...
        const donGia = parseFloat(cells[5].innerText.replace(/[^\d]/g, ''));
