    ...
    fetch(`../controller/pagTacGia.php?page=${page}`)
        ...
        .then(data => {
            ...
            data.data.forEach(author => {
                const row = `
                    <tr>
                        <td>${author.idthuonghieu}</td>
                        <td>${author.tenthuonghieu}</td>
                        <td>${author.gioithieu}</td>
                        <td>${author.trangthai == 1 ? "Hoạt động" : "Tạm khóa"}</td>
                        <td>
                            <a href="../controller/index.php?pg=suathuonghieu&idthuonghieu=${author.idthuonghieu}">
                                <img src="../view/layout/logo-icon/update-icon.png" alt="Sửa" style="width:20px; height:20px;">
                            </a>
                        </td>
                        <td>
                            <a href="../controller/index.php?pg=xoathuonghieu&idthuonghieu=${author.idthuonghieu}" onclick="return confirm('Bạn có chắc muốn xóa thương hiệu ${author.tenthuonghieu} không?')">
    ...
    fetch(`../controller/pagTacGia.php?search=${encodeURIComponent(keyword)}&page=${page}`)
        ...
        .then(data => {
            ...
            if (data.data.length === 0) {
                tbody.innerHTML = `
                    <tr>
                        <td colspan="6" style="text-align:center;">Không tìm thấy thương hiệu phù hợp!</td>
                    </tr>
                `;
