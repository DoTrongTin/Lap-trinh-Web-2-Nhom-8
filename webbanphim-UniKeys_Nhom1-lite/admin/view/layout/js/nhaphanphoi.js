    ...
    fetch(`../controller/pagNhaXuatBan.php?page=${page}`)
        ...
        .then(data => {
            ...
            data.data.forEach(publisher => {
                const row = `
                    <tr>
                        <td>${publisher.idnhaphaphoi}</td>
                        <td>${publisher.tennpp}</td>
                        <td>${publisher.email}</td>
                        <td>${publisher.sodienthoai}</td>
                        <td>${publisher.diachi}</td>
                        <td>${publisher.trangthai == 1 ? "Hoạt động" : "Tạm khóa"}</td>
                        <td>
                            <a href="../controller/index.php?pg=suanhaphanphoi&idnhaphaphoi=${publisher.idnhaphaphoi}">
                                <img src="../view/layout/logo-icon/update-icon.png" alt="Sửa" style="width:20px; height:20px;">
                            </a>
                        </td>
                        <td>
                            <a href="../controller/index.php?pg=xoanhaphanphoi&idnhaphaphoi=${publisher.idnhaphaphoi}" onclick="return confirm('Bạn có chắc muốn xóa nhà phân phối ${publisher.tennpp} không?')">
    ...
    fetch(`../controller/pagNhaXuatBan.php?search=${encodeURIComponent(keyword)}&page=${page}`)
        ...
        .then(data => {
            ...
            if (data.data.length === 0) {
                tbody.innerHTML = `
                    <tr>
                        <td colspan="8" style="text-align:center;">Không tìm thấy nhà phân phối phù hợp!</td>
                    </tr>
                `;
