<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {  
        ...
        if($result){
            echo "<script>alert('Tên thương hiệu đã tồn tại! Vui lòng nhập lại.');</script>";
        } else {
            if (themTacGia($tenTacGia, $tieuSu)) {
                echo "<script>alert('Thêm thương hiệu thành công!'); window.location.href = '../controller/index.php?pg=sanpham&tabId=tab3';</script>";
            } else {
                echo "<script>alert('Thêm thương hiệu thất bại! Vui lòng thử lại.');</script>";
            }
        }
    }
?>
        ...
        <div class="container">
            <form action="../controller/index.php?pg=themthuonghieu" method="post">    
                ...
                <input type="text" placeholder="Nhập tên thương hiệu" name="authorName" required
                    value="<?php echo htmlspecialchars($tenTacGia); ?>">
    
                <label for="bio"><b>Giới thiệu</b></label>
